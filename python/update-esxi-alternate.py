...
def scp_file(username, hostname, password, source, destination): # copy file to remote server based on local source, remote destination, server, username and hostname
    ...
    #            key = input('\033[01;33mIt appears the password is incorrect, would you like to re-enter it [y/n]?\033[00m ')
    #            if key == 'y':
    #                get_password()
    #                scp_process.close()
    #                scp_file(username, hostname, esxi_password, source, destination)
    #                return 
    #            else:
...
def check_maintenance_mode(ssh_connection):
    ...
    if not isinstance(ssh_connection, pexpect.spawn):
        ...
        #y = input('press y to check maintenance mode: ')
        #if y != 'y':
        #    exit(1)
        #print('ssh_connection.before: ' + ssh_connection.before + '\n\n\n\n\n\n\n\n\n')
        ssh_connection.sendline('vim-cmd hostsvc/hostsummary')
        match_prompt(ssh_connection)
        #print(ssh_connection.after)
...
def get_config_bundle():
    ...
    open('/home/' + os.getlogin() + '/Downloads/configBundle.tgz', 'wb').write(config_bundle_response.content)
    print('\033[01;36mESXi config bundle downloaded to: /home/' + os.getlogin() + '/Downloads/configBundle.tgz\033[00m')
...
def set_prompt(ssh_connection):
    if not isinstance(ssh_connection, pexpect.spawn):
        ...
        #y = input('press y to set the prompt: ')
        #if y != 'y':
        #    exit(1)
#        print('Showing ssh_connection.before/after before and after setting prompt\n\n\n')
#        print('before:before: ' + ssh_connection.before)
#        print('before:after: ' + ssh_connection.after)
        ssh_connection.sendline('unset prompt; unset PROMPT_COMMAND')
        ssh_connection.sendline('PS1="[PEXPECT]# "')
#        print('after:before: ' + ssh_connection.before)
#        print('after:after: ' + ssh_connection.after)
        match_prompt(ssh_connection)
        match_prompt(ssh_connection)
...
def match_prompt(ssh_connection):
    ...
    if not isinstance(ssh_connection, pexpect.spawn):
        ...
        prompt = '\[PEXPECT\]#'
        #print('Times prompt matched: ' + str(match_prompt_counter))
        #y = input('press y to match the prompt: ')
        #if y != 'y':
        #    exit(1)
        ...
        while True:
            ...
            if ssh_index == 0:
                ...
                if times_password_prompted > 1:
                    ...
                    ssh_index = ssh_connection.expect(['[Pp]assword', 'Are you sure you want to continue connecting', pexpect.TIMEOUT, pexpect.EOF])
                    print('\033[01;34mEnsuring password was correct...\033[00m')
                    if ssh_index == 0:
                        print('\033[01;33mPassword was not correct! Plase re-run this script with the correct password\033[00m')
                        exit(1)
                    else:
                        match_prompt(ssh_connection)
...
while True:
    ssh_index = ssh_connection.expect (['[Pp]assword','Are you sure you want to continue connecting', pexpect.EOF, pexpect.TIMEOUT, 'The update completed successfully','Host is not changed']) # start scanning stdout again, mostly listening for 'The update completed successfullly'
    if ssh_index == 0: 
